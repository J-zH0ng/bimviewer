<template>
    <div>
        <v-toolbar flat>
            <v-spacer></v-spacer>
            <v-toolbar-title>{{searchTerm}} 的搜索结果</v-toolbar-title>
            <v-spacer></v-spacer>
        </v-toolbar>
        <v-container>
            <v-row
...
export default {
    ...
    methods: {
      ...
      async pageChange() {
        this.curPage = this.results.slice((this.page - 1) * 16, this.page * 16)
      },
    ...
    async mounted() {
      ...
      const response = await axios.get(`${process.env.VUE_APP_BASE_API}/search`, {
        params: {
      ...
      this.curPage = this.results.slice(0, 16);
    },
}
</script>